...
func (srv *GoodsService) Get(id int) (rep models.GoodsRep, e error) {
...
func (srv *GoodsService) GetAll() (result []models.GoodsRep, e error) {
	objs, e := srv.GoodsRepo.GetAll()
	if e != nil {
		return
	}
	for _, goodsModel := range objs {
		result = append(result, models.GoodsRep{
			ID:    goodsModel.ID,
